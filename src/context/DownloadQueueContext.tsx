// src/context/DownloadQueueContext.tsx
...
const DownloadQueueContext = createContext<
  DownloadQueueContextType | undefined
>(undefined);
...
const downloadChapterContent = async (
  ...
  const instance = await Promise.race([
    ...
    new Promise((_, reject) =>
      setTimeout(
        () => reject(new Error("Plugin loading timeout (15s)")),
        15000,
      ),
    ),
  ]);
  ...
  while (attempts < maxAttempts && !chapterText) {
    try {
        ...
        new Promise((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(
                  `Chapter parsing timeout (30s) - attempt ${attempts + 1}`,
                ),
              ),
            30000,
          ),
      ...
      if (attempts >= maxAttempts) {
        throw new Error(
          `Failed to parse chapter after ${maxAttempts} attempts: ${e.message}`,
        );
      }
...
export const DownloadQueueProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const setTasksAndRef = useCallback(
    (updater: (prev: DownloadTask[]) => DownloadTask[]) => {
      setTasks((prev) => {
        ...
        const loaded = nextTasks
          .map((t: any) => {
            ...
            return {
              ...
              pluginName:
                t?.pluginName != null ? String(t.pluginName) : undefined,
              ...
              errorMessage:
                t?.errorMessage != null ? String(t.errorMessage) : undefined,
            };
          })
          .filter(
            (t: DownloadTask) =>
    ...
    (input: EnqueueInput | EnqueueInput[]) => {
      ...
      setTasksAndRef((prev) => {
        ...
        for (const item of list) {
          if (!item || !item.pluginId || !item.novelId || !item.chapterPath)
            continue;
  ...
  const cancelNovelTasks = useCallback(
    (novelId: string) => {
      if (!novelId) return;
      setTasksAndRef((prev) => {
  ...
  const startTask = useCallback(
    async (task: DownloadTask) => {
      ...
      if (!currentTask || currentTask.status !== "pending") {
        console.log("⏸️ Task is not pending, skipping:", {
          taskId: task.id,
            ...
            prev.map((t) =>
              t.id === task.id
                ? {
                    ...
                    errorMessage: "File system test failed",
                  }
                : t,
            ),
          );
      ...
      try {
        ...
        const html = await downloadChapterContent(
          task,
          settings,
          () => novelsRef.current,
        );
        console.log(
          "✅ STEP 1 COMPLETED: Chapter content loaded, length:",
          html.length,
        );
        ...
        console.log(
          "⏳ Showing completed status for 2 seconds before auto-removal",
        );
        await new Promise((resolve) => setTimeout(resolve, 2000));
        ...
        if (wasCanceled) {
          if (wroteFile) {
            try {
              ...
              console.log(
                "⚠️ Failed to delete canceled download file:",
                deleteError,
              );
            }
          }
  ...
  useEffect(() => {
    ...
    console.log("📊 Queue analysis:", {
      ...
      pendingByPlugin: Array.from(pendingByPlugin.entries()).map(([k, v]) => ({
        pluginId: k,
        taskId: v.id,
      })),
